...
<div class="min-h-screen bg-[#1f2937]">
    <div class="p-6 max-w-5xl mx-auto">
        <h1 class="text-3xl font-extrabold text-white mb-8 border-b-4 border-blue-500 inline-block pb-2">Canchas de Tenis 🎾</h1>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
            @forelse($canchas->where('tipo', 'tenis') as $cancha)
            ...
            @empty
                <p class="col-span-3 text-center text-gray-600">
                    No hay canchas de tenis registradas todavía.
                </p>
            @endforelse
        </div>
        ...
        <div class="mt-12">
            ...
            @php
                $reservasTenis = $reservas->filter(function($reserva) {
                    return $reserva->cancha->tipo === 'tenis';
                });
            @endphp

            @if($reservasTenis->isEmpty())
                <p class="text-gray-600">No tienes reservas de canchas de tenis.</p>
            @else
                <div class="overflow-x-auto bg-white shadow-xl rounded-xl">
                    <table class="min-w-full border-collapse">
                        ...
                        <tbody class="text-gray-700">
                            @foreach($reservasTenis as $reserva)
